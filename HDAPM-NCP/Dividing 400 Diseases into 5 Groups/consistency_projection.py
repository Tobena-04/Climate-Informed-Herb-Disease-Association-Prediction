...
class NSP(object):
    ...
    def ATCSP(self):
        ...
        modulus = np.where(modulus == 0, 1e-10, modulus)#把0替换成很小的数
        a=temp_matrix / modulus
        np.savetxt("hd.txt", a, fmt="%f", delimiter=',')
    ...
    def DrugSP(self):
        ...
        modulus=np.where(modulus==0,1e-10,modulus)
        a = temp_matrix / modulus

        np.savetxt("hd2.txt", a, fmt="%f", delimiter=',')
        return temp_matrix / modulus
    ...
    def network_NSP(self):
        result = np.nan_to_num(
            (np.nan_to_num(self.ATCSP()) + np.nan_to_num(self.DrugSP())) / self.calculate_modulus_sum())
        return result
